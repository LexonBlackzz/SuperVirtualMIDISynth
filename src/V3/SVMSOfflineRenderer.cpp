...
public:
    bool Initialize(const Options& o, std::string& error) {
        ...
        voices_.Initialize(maxVoices_,rate_); channels_.Reset(); channels_.SetMasterVolume(master_);
...
private:
    ...
    void Control(uint8_t ch,uint8_t cc,uint8_t value){const bool sustain=channels_.IsSustainActive(ch);channels_.ControlChange(ch,cc,value);if(cc==0||cc==32)RefreshPreset(ch);if(cc==64&&value<64){const uint32_t n=voices_.GetChannelActiveCount(ch);const uint32_t* h=voices_.GetChannelActiveList(ch);for(uint32_t i=0;i<n;++i)if(voices_.v.heldBySustain[h[i]]){voices_.v.heldBySustain[h[i]]=0;voices_.StartRelease(h[i]);}}
        if(cc==120)voices_.SilenceChannelImmediate(ch);else if(cc==123)voices_.ReleaseChannel(ch,0);else if(cc==121&&sustain){const uint32_t n=voices_.GetChannelActiveCount(ch);const uint32_t* h=voices_.GetChannelActiveList(ch);for(uint32_t i=0;i<n;++i)if(voices_.v.heldBySustain[h[i]]){voices_.v.heldBySustain[h[i]]=0;voices_.StartRelease(h[i]);}}
